...
export async function GET(
  ...
  try {
    ...
    if (segments.length === 0) {
      return new Response("PDF path is required", { status: 400 });
    }
    ...
    const looksLikePdf =
      nodePath.extname(resolvedFilePath).toLowerCase() === ".pdf" ||
      (await isPdfFile(resolvedFilePath));

    if (!looksLikePdf) {
      return new Response("Requested file is not a PDF", { status: 415 });
    }

    const fileName = nodePath.basename(resolvedFilePath).replace(/"/g, "");
    ...
    return new Response("Unable to serve PDF", { status: 500 });
  }
}
